...
public class SearchServiceImpl implements SearchService {
    ...
    public List<String> getBooks() {
        Callable<String> task = () -> restTemplate.getForObject("http://localhost:8801/library/book", String.class);
    ...
    @Override
    @HystrixCommand(fallbackMethod = "fallbackAuthors")
    public List<String> getAuthors() {
        Callable<String> task = () -> restTemplate.getForObject("http://localhost:8801/library/author", String.class);
        Future<String> future = executorService.submit(task);
        try {
            return Arrays.asList(future.get());
        } catch (Exception e) {
            return Arrays.asList(fallbackAuthors());
        }
    }
    ...
    public List<String> getUniversityDetails() {
        Callable<String> task = () -> restTemplate.getForObject("http://localhost:8800/university", String.class);
        Future<String> future = executorService.submit(task);
        try {
            return Arrays.asList(future.get());
        } catch (Exception e) {
